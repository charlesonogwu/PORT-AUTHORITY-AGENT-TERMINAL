...
#
# What it does:
#   1. Verifies Node.js >= 22.4 and npm are present (CDP/BiDi use native WebSocket).
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
  ...
  Write-Host "    Install Node 22.4 or newer from https://nodejs.org/ and re-run this installer."
  exit 1
}
...
if ($nodeMajor -lt 22 -or ($nodeMajor -eq 22 -and $nodeMinor -lt 4)) {
  Write-Err "Node $nodeVer is too old. portpilot requires Node 22.4 or newer for CDP/WebDriver BiDi."
  exit 1
}
...
Write-Host "    2. The native dashboard app opens on its own — no URL to visit."
Write-Host ""
Write-Host "  Other:"
