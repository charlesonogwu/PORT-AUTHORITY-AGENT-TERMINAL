import { DEFAULT_SESSION_ID, laneSessionId } from "../core/lane.js";
const COLS = [
...
export function formatLanesTable(lanes) {
    const columns = [{ key: "id", label: "PPID", width: Math.max(4, ...lanes.map((lane) => lane.id.length)) }, ...COLS];
    const header = columns.map((c) => pad(c.label, c.width)).join("  ");
    const rows = lanes.map((lane) => {
        return columns.map((c) => {
            let value;
            if (c.key === "appPortStr")
                value = lane.appPort ? String(lane.appPort) : "-";
            else if (c.key === "chromePortStr")
                value = lane.chromeDebugPort ? String(lane.chromeDebugPort) : "-";
...
export function formatReserveBlock(lane) {
    const lines = [];
    ...
    if (typeof lane.appPort === "number")
        lines.push(`  App port:        ${lane.appPort}`);
    if (typeof lane.chromeDebugPort === "number")
        lines.push(`  Chrome debug:    ${lane.chromeDebugPort}`);
    lines.push(`  Chrome profile:  ${lane.chromeProfileDir}`);
    if (lane.task)
        lines.push(`  Task:            ${lane.task}`);
    lines.push("");
    if (typeof lane.chromeDebugPort === "number") {
        ...
        if (typeof lane.appPort === "number")
            lines.push(`  $env:APP_PORT="${lane.appPort}"`);
        lines.push("");
        lines.push("Bash:");
        ...
        if (typeof lane.appPort === "number")
            lines.push(`  export APP_PORT=${lane.appPort}`);
        lines.push("");
