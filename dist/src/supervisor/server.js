...
import { SUPERVISOR_PROTOCOL_VERSION, assertLaneId, } from "./protocol.js";
async function loadOrCreateToken(path) {
    await mkdir(dirname(path), { recursive: true });
    const validate = (value) => {
        const token = value.trim();
        if (!/^[a-f0-9]{64}$/i.test(token))
            throw new Error(`invalid PortPilot supervisor token file: ${path}`);
        return token;
    };
...
async function acquireUnixSupervisorLock(endpoint) {
    ...
    for (;;) {
        ...
        try {
            ...
            let ownerPid = 0;
            try {
                ownerPid = Number((await readFile(lockPath, "utf8")).split(":", 1)[0]);
            }
            catch { /* reclaim invalid lock */ }
            if (ownerPid > 0) {
                try {
                    process.kill(ownerPid, 0);
                    const busy = new Error("PortPilot supervisor is already running");
                    busy.code = "EADDRINUSE";
                    throw busy;
                }
                catch (processError) {
                    if (processError.code === "EADDRINUSE")
                        throw processError;
                }
            }
...
export async function startSupervisorServer(options) {
    ...
    if (process.platform !== "win32") {
        ...
        if (live) {
            await releaseLock();
            const error = new Error("PortPilot supervisor is already running");
        ...
        socket.on("data", async (chunk) => {
            ...
            try {
                ...
                if (!tokenMatches(token, request.token))
                    throw new Error("unauthorized supervisor request");
                if (request.protocolVersion !== SUPERVISOR_PROTOCOL_VERSION)
                    throw new Error("unsupported supervisor protocol version");
