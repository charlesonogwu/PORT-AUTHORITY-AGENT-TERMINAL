...
export async function closeBrowserForLane(lane, deps = {}) {
    const scanNow = deps.scan ?? (async () => ({ observations: await scanNative(), source: "native", errors: [] }));
    ...
    if (lane.browserPid !== undefined && lane.browserPid !== pid) {
        throw new Error(`refusing to close lane ${lane.id}: recorded pid ${lane.browserPid} does not match observed ${pid}`);
    }
    if (lane.browserStartedAt !== undefined &&
    ...
    if (finalVerdict.kind !== "safe-attach" ||
