 ...
 */
export function pickTab(tabs, ref) {
    if (tabs.length === 0)
        throw new PageControlError("browser has no open tabs");
...
function assertNavigableUrl(url) {
    if (!isSafeInitialUrl(url)) {
        throw new PageControlError(`refusing to navigate to "${url}" — URLs must use http/https/about/file/view-source/data scheme and must not begin with "-".`);
    }
}
async function defaultShotPath(lane) {
    const dir = join(portpilotHome(), "shots");
    await mkdir(dir, { recursive: true });
    return join(dir, `${lane.id}-${Date.now()}.png`);
}
/** ── Firefox: WebDriver BiDi ─────────────────────────────────────────────── */
class BidiPageController {
    client;
    lane;
    browser = "firefox";
    constructor(client, lane) {
        this.client = client;
        this.lane = lane;
    }
    async resolveContext(tabId) {
        ...
        if (contexts.length === 0)
            throw new PageControlError("Firefox has no open tabs");
        if (!tabId)
            return contexts[0].id;
        if (contexts.some((c) => c.id === tabId))
            return tabId;
    ...
    async tabs() {
        ...
        for (const c of contexts) {
            let title;
            try {
                const meta = (await this.client.evalJson(c.id, metaExpr()));
                title = meta.title;
            }
            catch {
    ...
    async navigate(url, tabId) {
        ...
        const meta = (await this.client.evalJson(ctx, metaExpr()));
        return { url: meta.url, title: meta.title };
    }
    async evalExpression(expression, tabId) {
        const ctx = await this.resolveContext(tabId);
        return this.client.evalJson(ctx, evalWrapperExpr(expression));
    }
    async text(selector, tabId) {
        const ctx = await this.resolveContext(tabId);
        return (await this.client.evalJson(ctx, textExpr(selector, TEXT_CAP_CHARS)));
    }
    async click(selector, tabId) {
        const ctx = await this.resolveContext(tabId);
        return (await this.client.evalJson(ctx, clickExpr(selector)));
    }
    async fill(selector, value, tabId) {
        const ctx = await this.resolveContext(tabId);
        return (await this.client.evalJson(ctx, fillExpr(selector, value)));
    }
    async screenshot(outFile, tabId) {
        const ctx = await this.resolveContext(tabId);
...
class CdpPageController {
    ...
    async resolveTarget(tabId) {
        ...
        if (pages.length === 0)
            throw new PageControlError("browser has no open page tabs");
        const picked = pickTab(pages.map((p) => (p.title === undefined ? { id: p.id, url: p.url } : { id: p.id, url: p.url, title: p.title })), tabId);
    ...
    async navigate(url, tabId) {
        ...
        const meta = (await client.evalJson(metaExpr()));
        return { url: meta.url, title: meta.title };
    }
    async evalExpression(expression, tabId) {
        const client = await this.clientFor(tabId);
    ...
    async text(selector, tabId) {
        const client = await this.clientFor(tabId);
        return (await client.evalJson(textExpr(selector, TEXT_CAP_CHARS)));
    }
    async click(selector, tabId) {
        const client = await this.clientFor(tabId);
        return (await client.evalJson(clickExpr(selector)));
    }
    async fill(selector, value, tabId) {
        const client = await this.clientFor(tabId);
        return (await client.evalJson(fillExpr(selector, value)));
    }
    async screenshot(outFile, tabId) {
        const client = await this.clientFor(tabId);
 ...
 */
export async function openPageController(lane) {
    ...
    if (check.verdict.kind === "safe-free") {
        throw new PageControlError(`lane ${lane.id} has no browser running on port ${port}. Launch it first (open / launch_browser_lane).`);
    }
    if (check.verdict.kind !== "safe-attach") {
        throw new PageControlError(`refusing to control port ${port}: ${check.verdict.kind} — the process there is not this lane's browser/profile.`);
    }
    ...
    try {
        ...
        if (err instanceof CdpError)
            throw new PageControlError(err.message);
        throw err;
    }
}
