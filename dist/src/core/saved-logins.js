...
import { normalizeCwd, nowIso } from "./lane.js";
...
export function normalizeLoginWebsite(value) {
    const raw = value.trim();
    if (!raw || /\s/.test(raw))
        throw new Error("provide a website hostname or HTTP(S) URL");
...
async function verifyProfile(profile) {
    ...
    if (!lexical || lexical.startsWith("..") || isAbsolute(lexical))
        throw new Error("profile outside PortPilot profiles");
}
function accountNickname(value) {
    if (value === undefined)
        return undefined;
    const label = value.trim();
    if (!label || label.length > 80 || /[\x00-\x1f\x7f]/.test(label))
        throw new Error("provide an account nickname of 1–80 characters");
    return label;
}
export async function rememberSavedLogin(laneId, input) {
    if (input.confirmed !== true)
        throw new Error("saved login requires explicit user confirmation");
    ...
    await updateRegistry(async (lanes) => {
        ...
        if (!lane)
            throw new Error("unknown immutable PPID");
...
export async function findSavedLogins(input) {
    if (!input.cwd.trim())
        throw new Error("workspace cwd must not be blank");
    const cwd = normalizeCwd(input.cwd);
    const website = normalizeLoginWebsite(input.website);
    const accountLabel = accountNickname(input.accountLabel);
    const lanes = [];
    const unavailableProfileIds = [];
    for (const lane of await listLanes()) {
        if (normalizeCwd(lane.cwd) !== cwd || !(lane.savedLogins ?? []).some(l => l.website === website && (!accountLabel || l.accountLabel === accountLabel)))
            continue;
        lanes.push(lane);
        try {
            await verifyProfile(lane.chromeProfileDir);
            ...
            unavailableProfileIds.push(lane.id);
        }
    }
