...
async function lookupUnixProcesses(pids) {
    const byPid = new Map();
...
async function scanUnixNative(_opts) {
    ...
    try {
        ...
        if (res.stdout.trim()) {
            ...
            const lookup = await lookupUnixProcesses(observations.map((observation) => observation.pid).filter((pid) => typeof pid === "number"));
