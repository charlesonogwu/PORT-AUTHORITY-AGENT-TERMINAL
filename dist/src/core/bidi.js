...
export class BidiClient {
    ...
    static async connect(port, timeoutMs = 10_000) {
        ...
        try {
            ...
            if (/maximum number of active sessions/i.test(msg)) {
                throw new BidiError("Firefox already has an active BiDi session (a previous controller disconnected without session.end, " +
    ...
    async createContext() {
        ...
        if (!context)
            throw new BidiError("browsingContext.create returned no context id");
        return context;
    }
     ...
     */
    async evalJson(contextId, expression) {
    ...
    async screenshot(contextId) {
        ...
        if (!data)
            throw new BidiError("captureScreenshot returned no data");
        return data;
    }
    close() {
        try {
