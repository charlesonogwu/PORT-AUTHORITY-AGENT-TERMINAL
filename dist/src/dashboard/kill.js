 ...
 */
export async function killChromeByPid(pid) {
    ...
    try {
        ...
        const lane = lanes.find((l) => l.chromeDebugPort === port &&
    ...
    const killed = { pid };
    if (obs.command !== undefined)
        killed.command = obs.command;
    if (port !== undefined)
        killed.port = port;
    if (profileDir !== undefined)
        killed.profileDir = profileDir;
    const out = { ok: true, killed };
    if (releasedLaneId !== undefined)
        out.releasedLaneId = releasedLaneId;
    return out;
}
