...
export const EMPTY_PROCESS_SNAPSHOT = {
    processes: new Map(),
    connections: [],
};
/**
 ...
 */
export async function collectProcessSnapshot(opts = {}) {
    ...
    return new Promise((resolve) => {
        ...
        let child;
        try {
            child = spawn("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", COLLECT_SCRIPT], { stdio: ["ignore", "pipe", "pipe"], windowsHide: true });
        }
        catch {
        ...
        child.on("close", () => {
            ...
            try {
                ...
                if (!trimmed)
                    return settle(EMPTY_PROCESS_SNAPSHOT);
 ...
 */
export function sumTreeMemoryMB(rootPid, processes) {
    ...
    while (stack.length > 0) {
        ...
        if (mem > 0)
            sawMemory = true;
        bytes += mem;
        for (const child of children.get(cur.pid) ?? [])
            stack.push(child);
    }
    if (!sawMemory)
        return undefined;
...
export function parseSnapshot(raw) {
    ...
    for (const c of raw.connections ?? []) {
        if (typeof c.localPort !== "number" || typeof c.remotePort !== "number" || typeof c.owningPid !== "number")
            continue;
        connections.push({
