...
async function cmdLaunchChrome(ctx: CliContext): Promise<void> {
  ...
  if (ctx.json) emit(ctx, { ok: true, launched: !dryRun, lane, command: { binary: launch.binary, args: launch.args }, pid: launch.pid });
  else {
    if (dryRun) ctx.stdout.write(`Would launch: ${launch.binary} ${launch.args.join(" ")}\n`);
    else ctx.stdout.write(`Launched ${launch.binary} (pid=${launch.pid ?? "?"})\nDebug port: ${lane!.chromeDebugPort}\nProfile:    ${lane!.chromeProfileDir}\n`);
  }
}
...
async function cmdDashboard(ctx: CliContext): Promise<void> {
  ...
  if (!Number.isInteger(port) || port <= 0) fail(ctx, `invalid --port: ${port}`, 1);
  if (!isLoopbackHost(host) && !allowRemote) {
    fail(
      ctx,
      `refusing to bind dashboard to non-loopback host '${host}'. The dashboard ` +
        ...
        `accept that risk (e.g. on a trusted internal LAN).`,
      1,
    );
  }
  ...
  try {
    ...
    fail(ctx, `could not bind dashboard to ${host}:${port}: ${(err as Error).message}`, 1);
  }
  if (ctx.json) {
  ...
  if (!noOpen) openInBrowser(handle!.url);
