...
const MISSING_DEP_CODES = new Set<string>([
...
export function isMissingDependencyError(err: unknown): boolean {
  ...
  if (code && MISSING_DEP_CODES.has(code)) return true;
...
export function missingDependencyName(err: unknown): string | undefined {
 ...
 */
export function formatMissingDependencyMessage(opts: MissingDependencyMessageOpts): string {
  const dep = opts.missing ? ` (missing "${opts.missing}")` : "";
  return (
    `portpilot: the MCP server could not load its dependencies${dep}.\n` +
    `\n` +
