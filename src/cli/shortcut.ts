 ...
 */
async function stageDashboardBinary(): Promise<string> {
  const source = await resolveBundledBinary();
  if (!source) {
    throw new Error(
      `bundled ${bundledBinaryName()} not found in package. ` +
  ...
  try {
    ...
    if (code === "EBUSY") return destPath; // existing copy still works
    throw err;
  }
  // chmod +x on Unix so the launcher can spawn it.
  if (process.platform !== "win32") {
    const { chmod } = await import("node:fs/promises");
    try { await chmod(destPath, 0o755); } catch { /* ignore */ }
  }
  return destPath;
}
...
export async function installShortcut(opts: { port?: number; iconLocation?: string } = {}): Promise<ShortcutPaths> {
  if (process.platform !== "win32") {
    throw new Error(
      `portpilot shortcut is currently Windows-only. ` +
...
async function resolveBundledIcon(): Promise<string | null> {
  ...
  for (const c of candidates) {
    try { await access(c); return c; } catch { /* ignore */ }
  }
  return null;
}
...
export async function uninstallShortcut(): Promise<{ removedShortcut: boolean; removedLauncher: boolean }> {
  ...
  try { await rm(dashboardExe, { force: true }); removedLauncher = true; } catch { /* may be in use */ }
  try { await rm(legacyPs1, { force: true }); } catch { /* ignore */ }
  if (legacyGoExe) try { await rm(legacyGoExe, { force: true }); } catch { /* ignore */ }
