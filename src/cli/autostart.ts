...
function runPowerShell(script: string): Promise<{ ok: boolean; stderr: string }> {
...
async function resolveStartupFolder(): Promise<string> {
...
export async function uninstallAutostart(): Promise<{ removed: boolean; path: string }> {
  const paths = await autostartPaths();
  if (!paths.shortcut) return { removed: false, path: "" };
  let removed = false;
  try { await rm(paths.shortcut, { force: true }); removed = true; } catch { /* ignore */ }
  return { removed, path: paths.shortcut };
}

export async function autostartStatus(): Promise<{
  startup: string;
  shortcut: string;
  installed: boolean;
}> {
  ...
  let installed = false;
  if (paths.shortcut) {
    try { await access(paths.shortcut); installed = true; } catch { /* not installed */ }
  }
