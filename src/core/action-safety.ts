 ...
 */
export function evaluateLaneAction(
  lane: Lane,
  requestedPid: number,
  ...
  if (verdict.kind !== "safe-attach") {
    throw new Error(
      `refused dashboard action: lane ${lane.id} did not pass safe-attach (${verdict.kind})`,
    );
  }
  if (verdict.observation.pid !== requestedPid) {
    throw new Error(
      `refused dashboard action: live PID ${verdict.observation.pid ?? "unknown"} does not match requested PID ${requestedPid}`,
    );
  }
