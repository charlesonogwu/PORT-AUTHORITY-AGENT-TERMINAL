...
export function assertParses(expr: string): string {
...
export function metaExpr(): string {
  return assertParses(
...
export function textExpr(selector: string | undefined, maxChars: number): string {
 ...
 */
export function fillExpr(selector: string, value: string): string {
  return assertParses(
    `JSON.stringify((() => {
      const el = document.querySelector(${lit(selector)});
 ...
 */
export function evalWrapperExpr(userExpression: string): string {
