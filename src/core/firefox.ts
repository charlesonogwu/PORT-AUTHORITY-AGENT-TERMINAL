...
import { Lane, normalizeCwd, validatePortRange } from "./lane.js";
...
export class UnsupportedFirefoxModeError extends Error {
  constructor(mode: string) {
    super(
      `Firefox does not support mode="${mode}". Firefox has no off-screen window ` +
...
const FIREFOX_FAMILY_BASENAMES = new Set<string>([
  "firefox.exe",
  "firefox",
  "firefox-bin",
  "firefox-esr",
...
export function isFirefoxBinaryPath(p: string | undefined): boolean {
  if (!p) return false;
  return FIREFOX_FAMILY_BASENAMES.has(basenameLower(p));
}

const DEFAULT_FIREFOX_BINARIES: Record<string, string[]> = {
  win32: [
  ...
  if (explicit && explicit.length > 0) {
    if (!isFirefoxBinaryPath(explicit)) {
      throw new UnsafeChromeArgError(
        `Refusing to launch "${explicit}" — basename is not a recognised Firefox-family binary. ` +
...
export function isFirefoxProcess(o: PortObservation): boolean {
 ...
 */
export function extractFirefoxProfileDir(commandLine: string | undefined): string | undefined {
 ...
 */
export function evaluateFirefoxAttach(lane: Lane, observations: PortObservation[]): ChromeAttachVerdict {
  ...
  if (!isFirefoxProcess(obs)) return { kind: "unsafe-unknown", port, observation: obs };
 ...
 */
export function buildFirefoxLaunchPlan(lane: Lane, opts: LaunchFirefoxOptions = {}): FirefoxLaunchPlan {
  if (typeof lane.chromeDebugPort !== "number") {
    throw new Error("Lane has no debug port assigned");
  }
  validatePortRange(
    { start: lane.chromeDebugPort, end: lane.chromeDebugPort },
    "browser debug",
  );
  const mode = opts.mode ?? "visible";
  if (mode === "background") throw new UnsupportedFirefoxModeError(mode);
  ...
  if (opts.initialUrl && opts.initialUrl.length > 0) {
    if (!isSafeInitialUrl(opts.initialUrl)) {
      throw new UnsafeChromeArgError(
        `Refusing to pass "${opts.initialUrl}" as Firefox's startup URL. ` +
...
export async function launchFirefoxForLane(lane: Lane, opts: LaunchFirefoxOptions = {}): Promise<LaunchResult> {
  const mode = opts.mode ?? "visible";
