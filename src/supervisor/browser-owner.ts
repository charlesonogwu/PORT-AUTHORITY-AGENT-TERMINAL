...
import type {
  ...
  SupervisorCloseResult,
  SupervisorHandlers,
  SupervisorLaunchRequest,
  SupervisorLaunchResult,
} from "./protocol.js";
...
function withIdentity(lane: Lane, args: {
  supervisorId: string;
  state: Lane["browserState"];
  pid?: number;
  startedAt?: string;
  clearIdentity?: boolean;
  replaceStartedAt?: boolean;
}): Lane {
...
export function createBrowserOwner(deps: BrowserOwnerDependencies): SupervisorHandlers {
  const laneLocks = new Map<string, Promise<void>>();

  async function withLaneLock<T>(laneId: string, operation: () => Promise<T>): Promise<T> {
    ...
    try {
      ...
      release();
      if (laneLocks.get(laneId) === tail) laneLocks.delete(laneId);
    }
  }

  async function requireLane(id: string): Promise<Lane> {
    const lane = await deps.getLane(id);
    if (!lane) throw new Error(`lane not found: ${id}`);
    if (lane.status === "released") throw new Error(`lane is released: ${id}`);
    return lane;
  }

  return {
    async launch(request: SupervisorLaunchRequest): Promise<SupervisorLaunchResult> {
      return withLaneLock(request.laneId, async () => {
        const lane = await requireLane(request.laneId);
        const controller = new AbortController();
        const timer = setTimeout(
          () => controller.abort(new Error("browser launch verification deadline exceeded")),
          deps.operationTimeoutMs ?? 12_000,
        );
        timer.unref?.();
        ...
        try {
          ...
          if (verdict.kind !== "safe-free") throw new Error(`unsafe to launch lane ${lane.id}: ${verdict.kind}`);
          ...
          let verified: ChromeAttachVerdict | undefined;
          const attempts = deps.verifyAttempts ?? 100;
          for (let attempt = 0; attempt < attempts; attempt += 1) {
            controller.signal.throwIfAborted();
            verified = await deps.check(lane, controller.signal);
            controller.signal.throwIfAborted();
            if (verified.kind === "safe-attach") break;
            if (verified.kind !== "safe-free") throw new Error(`launched browser failed identity verification: ${verified.kind}`);
            if (attempt + 1 < attempts) await waitForRetry(deps.verifyDelayMs ?? 100);
          }
    ...
    async close(request: SupervisorCloseRequest): Promise<SupervisorCloseResult> {
      return withLaneLock(request.laneId, async () => {
        const lane = await requireLane(request.laneId);
