 ...
 */
function buildFocusScript(pid: number): string {
  return [
 ...
 */
function buildHideScript(pid: number): string {
  return [
    '$ErrorActionPreference = "SilentlyContinue"',
    `$pid_arg = ${pid}`,
    "$proc = Get-Process -Id $pid_arg -ErrorAction SilentlyContinue",
...
function runPowerShell(script: string, timeoutMs: number): Promise<{ ok: boolean; stdout: string; stderr: string; code: number | null }> {
...
export async function focusChromeWindow(pid: number, opts: { timeoutMs?: number } = {}): Promise<FocusResult> {
  ...
  if (out === "ok") return { ok: true, pid };
  if (out === "no-process") return { ok: false, error: "process exited before we could focus it" };
  if (out === "no-window") return { ok: false, error: "this Chrome has no window (headless or helper subprocess)" };
  if (out === "set-foreground-failed") return { ok: false, error: "Windows refused the focus request (try clicking the dashboard, then clicking Show again)" };
  if (out === "add-type-failed") return { ok: false, error: "could not load Win32 focus API" };
...
export async function hideChromeWindow(pid: number, opts: { timeoutMs?: number } = {}): Promise<FocusResult> {
  ...
  if (out === "ok") return { ok: true, pid };
  if (out === "no-process") return { ok: false, error: "process exited before we could hide it" };
  if (out === "no-window") return { ok: false, error: "this Chrome has no window (headless or helper subprocess)" };
  if (out === "minimize-failed") return { ok: false, error: "Windows refused the minimize request" };
  if (out === "add-type-failed") return { ok: false, error: "could not load Win32 ShowWindowAsync API" };
