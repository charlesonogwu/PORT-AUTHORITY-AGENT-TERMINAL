...
interface Signature {
  agent: string;
  /** Returns true when the process record looks like this agent. */
  test: (rec: ProcessRecord) => boolean;
}
...
const SIGNATURES: Signature[] = [
  ...
  // path contains "codex".
  {
    agent: "codex",
    test: (r) =>
      /[\\/]codex[\\/](?:dist|bin|cli|lib|src)[\\/]/i.test(r.commandLine) ||
  ...
  // GitHub Copilot CLI / `gh copilot`.
  {
    agent: "copilot",
    test: (r) =>
      /@github[\\/]copilot/i.test(r.commandLine) ||
 ...
 * Chrome on the box.
 */
export function inferAgentFromLiveChrome(args: InferArgs, snap: ProcessSnapshot): AgentInference {
 ...
 */
export function findCdpPeers(
  port: number,
  chromePid: number | undefined,
  connections: import("./process-info.js").TcpConnection[],
): number[] {
  const out = new Set<number>();
  for (const c of connections) {
    if (c.remotePort !== port) continue;
    if (c.owningPid <= 0) continue;
    if (chromePid !== undefined && c.owningPid === chromePid) continue;
    out.add(c.owningPid);
  }
  return [...out];
}
...
function matchSignature(rec: ProcessRecord): Signature | null {
  for (const sig of SIGNATURES) {
    if (sig.test(rec)) return sig;
  }
  return null;
}

function formatEvidence(source: string, rec: ProcessRecord): string {
