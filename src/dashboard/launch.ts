...
export function bundledBinaryName(): string {
...
export function stagedBinaryName(): string {
...
export function stagedBinaryDir(): string {
  if (process.platform === "win32") {
    const localAppData = process.env.LOCALAPPDATA;
    if (localAppData) return join(localAppData, "PAAT");
  }
...
export async function resolveBundledBinary(): Promise<string | null> {
...
export async function resolveDashboardBinary(): Promise<string | null> {
...
export async function launchDashboard(): Promise<LaunchResult> {
  ...
  if (!binary) {
    return {
      ...
      binary: "",
      error:
        `paat-dashboard binary not found. ` +
