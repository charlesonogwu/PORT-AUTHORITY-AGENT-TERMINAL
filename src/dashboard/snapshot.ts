...
export interface RegistryStatus {
  ...
  empty: number;
  staleEntries: { laneId: string; agent: string; project: string; reason: string }[];
}
...
interface VersionInfo { Browser: string }

async function gatherCdp(port: number, timeoutMs: number): Promise<{ version?: string; tabs: CdpTab[]; error?: string }> {
  try {
    const [version, tabs] = await Promise.all([
      fetchJson<VersionInfo>(`http://127.0.0.1:${port}/json/version`, timeoutMs),
      fetchJson<CdpTab[]>(`http://127.0.0.1:${port}/json/list`, timeoutMs),
    ]);
    return {
    ...
    return { tabs: [], error: (err as Error).message };
  }
}
...
function isInternalTab(t: CdpTab): boolean {
  const url = t.url ?? "";
...
function buildLiveSession(
  live: LiveChrome,
  ppLane: UnifiedLane | undefined,
  cdp: { version?: string; tabs: CdpTab[]; error?: string },
  processSnap: ProcessSnapshot,
  births: BirthRegistry,
): LiveSession {
  const lane = ppLane;
  const registeredBy: EntrySource | null = ppLane ? "portpilot" : null;
  const profile = live.profileDir ?? "";

  let agent: string;
  let agentConfidence: Confidence;
  let project: string;
  let projectConfidence: Confidence;
  let cwd: string | undefined;
  let cwdConfidence: Confidence;
  let inferenceConfidence: "high" | "medium" | "low" | "none" | undefined;
  let inferenceEvidence: string[] | undefined;
  ...
  if (lane) {
    ...
    const inferred = inferAgentFromLiveChrome(
      { chromePid: live.pid, port: live.port, profileDir: profile || undefined, births },
      processSnap,
    );
  ...
  if (lane?.savedLogins !== undefined) session.savedLogins = lane.savedLogins;
  ...
  if (lane?.id) session.laneId = lane.id;
  if (inferenceConfidence) session.agentInferenceConfidence = inferenceConfidence;
  if (inferenceEvidence && inferenceEvidence.length > 0) session.agentInferenceEvidence = inferenceEvidence;
  return session;
}

function summarizeRegistry(
  lanes: UnifiedLane[],
  liveByPortAndProfile: Map<string, LiveChrome>,
  found: boolean,
  now: number,
): RegistryStatus {
  let live = 0;
  let stale = 0;
  let empty = 0;
  const staleEntries: RegistryStatus["staleEntries"] = [];
  for (const lane of lanes) {
    ...
    const sameLane = { status: lane.status, lastSeen: lane.lastSeen } as Parameters<typeof isStale>[0];
...
function detectConflicts(
  ...
  for (const lane of active) {
    ...
    if (!live) continue;
    if (profilesEqual(live.profileDir, lane.chromeProfileDir)) continue;
    conflicts.push({
      kind: "registry-mismatch-with-live",
      port: lane.chromeDebugPort,
      message:
        `port ${lane.chromeDebugPort}: portpilot reserves for ${lane.owner}/${lane.project}, ` +
  ...
  const byPort = new Map<number, UnifiedLane[]>();
  for (const lane of active) {
    if (typeof lane.chromeDebugPort !== "number") continue;
...
export async function buildSnapshot(opts: { cdpTimeoutMs?: number } = {}): Promise<DashboardSnapshot> {
  ...
  for (const live of liveChromes) {
    if (!live.pid || !live.profileDir) continue;
    if (births.has(live.pid, live.profileDir)) continue;
  ...
  births.flush().catch(() => { /* logged elsewhere */ });
  ...
  const liveByPortAndProfile = new Map<string, LiveChrome>();
